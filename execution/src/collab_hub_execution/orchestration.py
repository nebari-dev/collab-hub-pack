...
class DurableWorkflowEngine(WorkflowEngine):
    ...
    def _step_attempts(self, run_id: str, step: str) -> int:
        return sum(
            1
            for e in self.track.replay(run_id)
            if e.event_type == "step_started" and e.payload.get("step") == step
        )
    ...
    def _advance(self, op: OpDefinition, signal: Any = None) -> RunStatus:
        ...
        for step in op.steps:
            ...
            attempt = self._step_attempts(op.run_id, step.name)
            ...
            try:
                ...
                if worker is not None:
                    try:
                        ...
                        self._append(op.run_id, "teardown_failed", step=step.name, error=type(exc).__name__)
