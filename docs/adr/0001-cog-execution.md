...
**D9 — Manifests follow cog-spec as a compatible profile, hosted in `pixi.toml`.** `COG.md` identity envelope → `pixi.toml` (the environment *and* the OpenTeams profile manifest, under `[tool.cog]`: entry points, io, model/harness `requires`, frames, connectors) → `context/`. Because v1 Cogs are Nebi-distributed (D6), the profile puts its declarations in the one file Nebi already reads rather than a second file that must be kept in step with it; `version`, `description`, and the tasks that entry points name are then stated once. `COG.md` still names the manifest (`manifest: pixi.toml`) as cog-spec[^3] requires — that pointer, not the filename, is what keeps the profile compatible: a Cog that is not a pixi workspace may point at another file, and readers dispatch on the pointer. Our profile extends cog-spec rather than forking it; extensions are upstreamed. The `requires`/connector declarations stay structured objects — never flattened to strings — so the data catalog can attach source ratings to them later (D10).
