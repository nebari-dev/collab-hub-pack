...
|---|---|---|
| `docs/adr/0002-…`, `docs/adr/README.md` | the decisions | 0 (done); 3 appends decision 18, the state pattern; 8 appends D11, agent location; each decision in §10 as it settles |
...
| 3 | #130 | State machines for the Cog, the worker, the step attempt and the run, on the state pattern | `feat/cog-state-machines` | 2 | M | not started |
...
**Issue** #130 · **Branch** `feat/cog-state-machines` · **Depends on** Phase 2 · **Size** M
...
- **Pure transitions.** A transition takes an event and returns the next state and the Track events that record it; it performs no I/O, reads no clock and calls no executor. The Lifecycle runner applies it (Phase 6) and a durability backend schedules the runner, so every backend moves the same machines. Replaying a run's Track events through the run machine is how its status is derived (Phase 5) — the one derivation the API, the controller and the desktop's Local run host share (ADR-0002 D3).
- **Guards live in the state that owns them**, for every transition §11 labels with a condition: retry from `INTERRUPTED` keeps the attempt and its idempotency key, from `FAILED` opens a new attempt and key, from `BUDGET_EXCEEDED` a new budget epoch; `OUTCOME_UNKNOWN` leaves only through reconciliation; a decision must name the open escalation; a send back past the revise limit ends the run `FAILED` with reason `revise_limit_exceeded`.
...
- #35's `CogLifecycle` transition table and the statuses in `RunStatus` are replaced by the worker and run machines. Wire values are the state names in lower case — `waiting_at_gate` — and #35's `paused` and `timed_out` are read as `waiting_at_gate` and `budget_exceeded` with dimension `duration` by the Track's pre-v1 reader (Phase 5).
...
| `UNINSTALLED` | the Cog | The install and every runtime resource it created are removed, its warm pool drained. Reachable from any install state, so an install stuck before `INVOKABLE` can be removed. | Phases 20, 22 |
...
| `WAITING_AT_GATE` | a run | A step's Gate escalated; the run waits for a decision naming the open escalation id. A send back re-runs the step, bounded by the revise limit, past which the run ends `FAILED` with reason `revise_limit_exceeded`. `paused` in #35's code, where the Cog asked for it; Phase 4 moves the decision to the step. | `RunStatus.PAUSED`; Phases 4, 11 |
